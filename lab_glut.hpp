...
#pragma once
#ifndef _LAB_GLUT_HPP
#define _LAB_GLUT_HPP
...
namespace lab{ namespace glut{
        ...
        #if defined(__APPLE__) && defined(OLD_GLUT)
            if(!context.core) std::cout<<"ATENTIE: Din cauza GLUT-ului de pe OS X, fortam OpenGL 3.2 Core Profile"<<std::endl;
            ContextInfo appleContext(3, 2, true);
            _context_information = appleContext;
			glutInitDisplayMode(framebuffer.flags | GLUT_3_2_CORE_PROFILE);	
        #else
        ...
        #if defined(__APPLE__) && defined(OLD_GLUT)
			glutWMCloseFunc(_closeCallback);
        #else
        ...
        #if defined(OLD_GLUT) && !defined(__APPLE__)
            glutMainLoop();
        #else
            _running = true;
            while (_running) {
...
#ifdef OLD_GLUT
            std::cout<<"EROARE! Acest GLUT nu suporta iesirea din Full Screen"<<std::endl;
#else
			glutLeaveFullScreen();
#endif
		}
